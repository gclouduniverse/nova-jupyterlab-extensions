...
export class ButtonExtension implements
  ...
  createNew(
    ...
    let callback = () => {
      ...
      let path_to_folder =
          PageConfig.getOption('serverRoot') + '/' + notebook_path
      path_to_folder =
          path_to_folder.substring(0, path_to_folder.length - notebook.length);
...
class SubmitJobForm extends Widget {
  ...
  private static createFormNode(): HTMLElement {
    ...
    var trainingTargets = [
      'gce',
      // "local" - local trainig is not yet supported and should be reanabled as
    ...
    return {
      ...
      'instance_type':
          (<HTMLInputElement>this.node.querySelector('#instanceTypeInput'))
              .value,
      'local': (<HTMLInputElement>this.node.querySelector('#trainingTypeInput'))
                   .value == 'local',
      'parameter':
