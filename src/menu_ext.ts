...
function activate(
  ...
  app.commands.addCommand(overnightCommand, {
    ...
    execute: () => {
      ...
      if (!found) {
        files.defaultBrowser.model.manager
            .newUntitled({path: '', type: 'directory'})
            .then(
                model => {files.defaultBrowser.model.manager.rename(
                    'Untitled Folder', 'jobs')})
      }


      if (currentWidget) {
        files.defaultBrowser.model.manager.copy(
            currentWidget.context.session.path, 'jobs')
