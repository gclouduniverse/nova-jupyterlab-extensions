...
const HTTP_TRIGGER_LINK = 'https://cloud.google.com/functions/docs/calling/http';
...
export class CloudFunctionDeployer extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div className={css.column}>
        <p>
          Your project must have an <LearnMoreLink href={HTTP_TRIGGER_LINK}
            text='HTTP-triggered Cloud Function' /> in order to submit Notebooks
          for scheduled execution. Click the Deploy button to provision the
          necessary function in the <em>{this.props.region}</em> region.
        </p>
        {message && <Message
        ...
        {<div className={css.actionBar}>
          <button className={css.button} onClick={this.props.onDialogClose}>
            Cancel</button>
          <SubmitButton actionPending={deploying}
  ...
  private async _onDeploy() {
    ...
    this.setState({
      deploying: true,
      message:
        `Deploying Cloud Function in ${region}. This may take a few minutes...`,
      hasError: false,
    });
    try {
